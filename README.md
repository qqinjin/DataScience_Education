...
- 날씨 정보:
  1. 좌표기반 API를 통해 선박 위치의 날씨 정보를 요청.
  2. 화재 위험도 보조지표로 활용.
- 위험도 판단:
  1. 기상 데이터와 머신러닝 모델을 통한 위험도 그래프 제작.
  2. 선박 위치 기반의 기상 정보 및 안전 점검 정보 표시.
- 웹 대쉬보드:
  1. HTML, CSS로 제작된 대쉬보드에서 실시간 정보 확인.
  2. 로컬 Flask 서버에서 실행하며, 중요 데이터는 AWS에 저장.
- 화재 대응 기능:
  1. 화재경보: 경광등 점등 및 경고음 실행.
  2. 수동 소화: 아두이노를 이용한 소화 노즐 제어 및 경광등 점멸.
  3. 구조 및 신고: 텔레그램 챗봇 알림 및 TTS 사운드 발생.
  4. 비화재 상황: 알림 초기화 및 데이터 저장.
- 서버 구조:
  1. Flask(선박 로컬서버): 선박 내부에서 데이터 처리 및 대쉬보드 제공.
  2. AWS(육지 서버): 안정적인 데이터 백업 및 고성능 처리. 외부 사용자에게 웹 서비스 제공 및 챗봇 서비스 연동.
...
<table>
  ...
  <tr>
    <td><img src="https://github.com/qqinjin/DataScience_Education/assets/99711238/57973ca7-b409-40c2-98a3-8c9cf114b4de)" alt="AWS데이터저장" width="500" height="400" /></td>
    <td><img src="https://github.com/qqinjin/DataScience_Education/assets/99711238/77827c81-4410-4b2e-8852-217e49e4dc99)" alt="텔레그램챗봇" width="250" height="400" /></td>
  </tr>
</table>
...
## 파일 
- 선박화재예측_딥러닝_모델 폴더: Yolo, Cylce-GAN의 이미지 전치리, 모델 학습 코드
- 선박화재예측_머신러닝_모델 폴더: 데이터 전처리 코드와 머신러닝 학습 코드
- 웹대쉬보드: 웹페이지 코드와 화재위험도 분석을 사용할 수 있도록
- 화재검출서비스: 플라스크 서버와 텔레그램 챗봇, 아두이노 코드
