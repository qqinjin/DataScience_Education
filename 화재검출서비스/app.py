...
# YOLOv7 모델 로드
model = torch.hub.load(r'C:\Users\jae12\Desktop\pycode\화재검출서비스\yolov5', 'custom', 'yolov5/best.pt', source='local')
...
def toggleSiren():
    ...
    siren_process = subprocess.Popen('dist/siren2.exe')
    return jsonify({'status': 'success'})
...
def Sprinkler():
    ...
    Sprinkler_process = subprocess.Popen('dist/filmata/water.exe')
    return jsonify({'status': 'success'})
...
def detect_objects():
    ...
    while True:
        ...
        for i in range(100):
            ...
            center_y = int(frame.shape[0] * 5/8)
            x1 = center_x - int(frame.shape[1] / 4)
            x2 = center_x + int(frame.shape[1] / 4)
            y1 = center_y - int(frame.shape[0] / 4)
            y2 = center_y + int(frame.shape[0] / 4)
            dframe = frame[y1:y2, x1:x2]
            results = model(dframe)
            if cnt == 0:
            ...
            if '0' in detected_classes:
                for detection in detections:
                    if detection[4] > 0.3 and classes[int(detection[5])] == 'fire':
                        ...
                        cv2.rectangle(dframe, (x1, y1), (x2, y2), (255, 0, 0), 2)
                        cv2.putText(dframe, 'fire', (x1, y1 - 10), cv2.FONT_HERSHEY_SIMPLEX, 0.9, (255, 0, 0), 2)
